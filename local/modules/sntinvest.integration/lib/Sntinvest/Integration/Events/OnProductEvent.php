...
use Sntinvest\Integration\Services\ProductService;
 ...
 * @author dimay
 */
class OnProductEvent implements EventInterface
{
    private $service;
    ...
    public function handler($param)
    {
                ...
                && $param['RESULT'])
        {
            ...
            $arFilter = ["IBLOCK_ID"=>intval($param['IBLOCK_ID']), "ID"=>intval($param['ID'])];
            
            $res = \CIBlockElement::GetList([], $arFilter, false, false, static::$select);
            
            while($ob = $res->GetNextElement())
            {
                $data = $this->getFields($ob->GetFields());
                $data['PROPERTY_VALUES'] = $this->GetProperties($ob->GetProperties());
            }
