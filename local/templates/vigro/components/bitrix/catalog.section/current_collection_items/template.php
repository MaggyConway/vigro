...
if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS'])) {
	$areaIds = array();
?>
	<div class="new-goods__slider">

		<?
		foreach ($arResult['ITEMS'] as $key => $item) {

			if ($item['PROPERTIES']['COLLECTION']['VALUE'] === $arParams['CURRENT_COLLECTION_ID']) {

				// echo "<pre>";
				// var_dump($item['PROPERTIES']['COLLECTION']['VALUE']);
				// echo "</pre>";
				...
				$productTitle = array_shift($array_words);
				// $category = array_slice($array_words, 1, 3);
				$category = implode(" ", $array_words);
		...
		?>
				<a href="<?= $item['DETAIL_PAGE_URL'] ?>" class="product-item new-goods__slider__item">
					...
					<? } ?>
					<img src="<?= $image ?>" alt="product_image" loading="lazy" />
