...
if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS'])) {
	...
	// echo $arParams['SLIDER_TYPE'];
?>

<div class="catalog_sliders__<?=$arParams['SLIDER_TYPE']?> pb-4 pb-sm-5<?= ($arParams['SLIDER_TYPE'] == 'hits') ? ' show' : '' ?>">
		...
		foreach ($arResult['ITEMS'] as $key => $item) {
			...
			$array_words = array();
			$separator = " ";
			$tok = strtok($item['NAME'], $separator);
			while ($tok) {
				$array_words[] = $tok;
				$tok = strtok(" ");
			}
			...
			$arr_price = CPrice::GetBasePrice($item['ID']);
			$price = intval($arr_price["PRICE"]);
		...
		<div>
			<div class="catalog_item">
				...
				<p class="catalog_item__title">
					...
					<span class="wish <?=Silab\SiteCore\Services\WishlistStateService::getState($item['ID'])?>" data-product-id="<?=$item['ID']?>"></span>
				</p>
