...
if (isset($arResult['ITEM'])) {
	...
	// $global_sections = [];
	// $sect_select = CIBlockElement::GetElementGroups(
	// 	$arResult["ITEM"]['ID'], 
	// 	true, 
	// 	array("ID", "SORT", "NAME", "CODE"));

	// while($ar_group = $sect_select->Fetch()) {
	// 	$global_sections[] = $ar_group;
	// }

	$glob_sect_code = explode('/', $arResult["ITEM"]["DETAIL_PAGE_URL"]);

	$data_code  = $arResult['PARENT_SECTION_CODE'] ?? $glob_sect_code[2];

	// echo '<pre>';
	// var_dump($glob_sect_code[2]);
	// echo '</pre>';
/*?>

	<div class="show-by-collection product-item-container<?= (isset($arResult['SCALABLE']) && $arResult['SCALABLE'] === 'Y' ? ' product-item-scalable-card' : '') ?>" id="<?= $areaId ?>" data-entity="item" data-code="<?=$data_code?>">
		<?*/
